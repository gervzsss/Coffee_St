          ...
          <?php foreach ($data['items'] as $it):
            ...
            <li class="flex items-start gap-3 py-3 sm:items-center">
              <div class="h-14 w-14 flex-shrink-0 overflow-hidden rounded-md bg-neutral-100 ring-1 ring-black/5">
                <?php if (!empty($img)): ?>
                  <img src="<?php echo htmlspecialchars($img, ENT_QUOTES); ?>" alt="<?php echo htmlspecialchars($p?->name ?? 'Product image'); ?>" class="h-full w-full object-cover" />
                <?php else: ?>
              ...
              <div class="flex min-w-0 flex-1 items-start justify-between gap-3">
                ...
                <div class="text-right">
                  ...
                  <?php if (!empty($it->price_delta) && (float)$it->price_delta !== 0.0): ?>
                    <div class="text-[11px] text-neutral-500">(Base ₱<?php echo number_format((float)$it->unit_price, 2); ?><?php echo ((float)$it->price_delta) > 0 ? ' +' : ' '; ?>₱<?php echo number_format(abs((float)$it->price_delta), 2); ?>)</div>
                  <?php endif; ?>
                </div>
              </div>
            </li>
    ...
    <aside>
      <div class="sticky top-28 rounded-lg border bg-white p-6 shadow-sm">
        ...
        <button id="place-order" class="btn-primary cursor-pointer mt-6 w-full text-center"
          ...
          data-tax="<?php echo htmlspecialchars((string) ($data['tax'] ?? 0), ENT_QUOTES); ?>"
          <?php if (!empty($data['selectedMode']) && !empty($data['selectedIds'])): ?>
            data-selected="<?php echo htmlspecialchars(implode(',', array_map('intval', $data['selectedIds'])), ENT_QUOTES); ?>"
          <?php endif; ?>
          <?php if (!empty($data['singleMode']) && !empty($data['singleProductId'])): ?>
            data-single-product-id="<?php echo (int) $data['singleProductId']; ?>" <?php endif; ?>>
          Place Order
        </button>
        <a href="/COFFEE_ST/public/pages/cart.php" class="mt-3 inline-flex w-full items-center justify-center gap-2 rounded border border-neutral-300 px-4 py-2 text-sm text-neutral-700 hover:bg-neutral-50">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5"/></svg>
          Back to Cart
        </a>
