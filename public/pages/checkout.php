  ...
  <main class="mx-auto max-w-5xl px-4 py-32">
    ...
    <?php if (empty($items)): ?>
      <div class="rounded-lg border bg-white p-6 shadow-sm">
        <p class="text-neutral-700">Your cart is empty. <a href="/COFFEE_ST/public/pages/products.php" class="text-[#30442B] underline">Continue shopping</a>.</p>
      </div>
    <?php else: ?>
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <section class="md:col-span-2 space-y-4">
          <div class="rounded-lg border bg-white p-6 shadow-sm">
            ...
            <ul class="divide-y">
              <?php foreach ($items as $it): $p = $productRepo->findById($it->product_id); ?>
                <li class="py-3 flex justify-between">
                  <span class="text-neutral-800"><?php echo htmlspecialchars($p?->name ?? ''); ?> × <?php echo (int)$it->quantity; ?></span>
        ...
        <aside>
          <div class="rounded-lg border bg-white p-6 shadow-sm sticky top-28">
            ...
            <dl class="space-y-1 text-sm">
              <div class="flex justify-between"><dt>Subtotal</dt><dd>$<?php echo number_format($subtotal,2); ?></dd></div>
              <div class="flex justify-between"><dt>Delivery Fee</dt><dd>$<?php echo number_format($deliveryFee,2); ?></dd></div>
              <div class="flex justify-between"><dt>Tax</dt><dd>$<?php echo number_format($tax,2); ?></dd></div>
            </dl>
            ...
            <button id="place-order" class="cursor-pointer mt-6 w-full py-3 rounded bg-[#30442B] text-white hover:bg-[#3d5a38] transition">Place Order</button>
  ...
  <script>
    $(function(){
      $('#place-order').on('click', function(){
        ...
        $.post('/COFFEE_ST/public/api/checkout.php', { delivery_fee: <?php echo json_encode($deliveryFee); ?>, tax: <?php echo json_encode($tax); ?> })
          .done(function(resp){
