...
if (!is_authenticated()) { header('Location: /COFFEE_ST/public/index.php'); exit; }

$orderId = isset($_GET['id']) ? (int)$_GET['id'] : 0;
$repo = new OrderRepository(db());
$orders = $repo->listForUser((int)current_user()['id']);
$order = null;
foreach ($orders as $o) { if ((int)$o->id === $orderId) { $order = $o; break; } }
if (!$order) { header('Location: /COFFEE_ST/public/pages/orders.php'); exit; }
...
<body class="min-h-screen bg-neutral-50 text-neutral-900 font-sans">
  ...
  <main class="mx-auto max-w-5xl px-4 py-32">
    ...
    <div class="rounded-lg border bg-white p-6 shadow-sm">
      <p class="text-sm text-neutral-600 mb-4">Placed on <?php echo htmlspecialchars(date('M j, Y g:i A', strtotime($order->created_at ?? 'now'))); ?> • Status: <span class="font-medium text-[#30442B] uppercase"><?php echo htmlspecialchars($order->status); ?></span></p>
      <table class="w-full text-sm">
        <thead class="text-neutral-500 border-b"><tr><th class="text-left py-2">Item</th><th class="text-right">Qty</th><th class="text-right">Unit</th><th class="text-right">Line</th></tr></thead>
        <tbody>
